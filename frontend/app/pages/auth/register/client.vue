...
<template>
  <NuxtLayout name="auth">
    <template #header>
      <h1 class="text-3xl font-serif font-bold text-nature-900 mb-2">Gabung Komunitas</h1>
      <p class="text-nature-600">Bikin akun biar bisa mulai jelajah.</p>
    </template>

    <template #quote>
      Mulai petualangan rasa otentikmu.
    </template>
    ...
    <div>
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <div>
          <label for="name" class="block text-sm font-medium text-nature-700 mb-1">Nama Lengkap</label>
          <input 
            id="name" 
        ...
        >
          {{ isLoading ? 'Lagi Bikin Akun...' : 'Bikin Akun' }}
        </button>
      </form>

      <div class="mt-8 text-center text-sm text-nature-600">
        Udah punya akun? 
        <NuxtLink to="/auth/login/client" class="text-leaf-600 hover:text-leaf-700 font-medium">Masuk Sini</NuxtLink>
      </div>
    </div>
  </NuxtLayout>
</template>
